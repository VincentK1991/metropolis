"""Handler for streaming messages via SSE with JSON formatting."""
...
class StreamHandler:
    """
    Handler for streaming messages via Server-Sent Events (SSE) with JSON formatting.
    ...
    def process_message(self, message: Any) -> list[dict[str, Any]]:
        """Process any message type and return JSON messages for SSE.

        Args:
            message: Message from the Claude SDK (StreamEvent, AssistantMessage, etc.)

        Returns:
            List of JSON-serializable dictionaries to send via SSE
        """
    ...
    def _create_tool_use_message(self, block: ToolUseBlock) -> dict[str, Any]:
        """Create a tool use message for SSE.
    ...
    def _create_tool_result_message(self, block: ToolResultBlock) -> dict[str, Any]:
        """Create a tool result message for SSE.
