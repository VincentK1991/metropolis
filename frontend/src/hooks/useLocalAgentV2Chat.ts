...
import {
  listLocalAgentV2Sessions,
  getLocalAgentV2SessionMessages,
  chatInLocalAgentV2,
  deleteLocalAgentV2Session,
  renameLocalAgentV2Session,
} from '../api/localAgentV2Service'
import type { ChatMessage, MessageContent } from '../types/chat'
import type { AgentV2Session, AgentV2SSEEventUnion } from '../types/agentV2'

interface UseLocalAgentV2ChatResult {
...
export const useLocalAgentV2Chat = (): UseLocalAgentV2ChatResult => {
  ...
  const loadAvailableSessions = useCallback(async (reset: boolean = false) => {
    try {
      ...
      const response = await listLocalAgentV2Sessions(sessionLimit, skip)
  ...
  const loadMoreSessions = useCallback(async () => {
    ...
    try {
      ...
      const response = await listLocalAgentV2Sessions(sessionLimit, sessionSkip)
  ...
  const loadSessionHistory = useCallback(async (sessionIdToLoad: string) => {
    try {
      ...
      const response = await getLocalAgentV2SessionMessages(
        sessionIdToLoad,
        messageLimit,
        0,
        true
  ...
  const deleteSession = useCallback(
    async (sessionIdToDelete: string) => {
      try {
        await deleteLocalAgentV2Session(sessionIdToDelete)
  ...
  const renameSession = useCallback(
    async (sessionIdToRename: string, newTitle: string) => {
      try {
        await renameLocalAgentV2Session(sessionIdToRename, newTitle)
          ...
          (data: AgentV2SSEEventUnion) => {
            ...
            if (data.type === 'session_created') {
              if (data.session_id) {
                ...
                localStorage.setItem('lastLocalAgentV2SessionId', data.session_id)
  ...
  useEffect(() => {
    ...
    const lastSessionId = localStorage.getItem('lastLocalAgentV2SessionId')
    if (lastSessionId) {
      switchToSession(lastSessionId).catch((err) => {
        console.error('Failed to load last session:', err)
        // Clear invalid session ID
        localStorage.removeItem('lastLocalAgentV2SessionId')
      })
    }
