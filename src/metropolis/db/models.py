...
class ContainerizedAgentSession(BaseModel):
    """
    Represents a containerized agent session.

    Similar to ClaudeAgentSession but for containerized agent v2 API.
    """

    id: Optional[str] = Field(default=None, alias="_id")
    claude_session_id: str = Field(
        ..., description="Session ID from containerized agent API"
    )
    created_at: datetime = Field(default_factory=lambda: datetime.now(UTC))
    updated_at: datetime = Field(default_factory=lambda: datetime.now(UTC))
    message_count: int = Field(default=0)
...
class ContainerizedAgentMessage(BaseModel):
    """
    Represents a single message in a containerized agent conversation.
