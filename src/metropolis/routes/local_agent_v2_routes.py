"""Local agent v2 routes for local testing integration."""
...
router = APIRouter(prefix="/api/v2/local-agent", tags=["local agent v2"])

# Global service instance (will be initialized in app.py)
_agent_service: Optional[LocalAgentService] = None
_store: Optional[LocalAgentStore] = None


def init_agent_service(service: LocalAgentService, store: LocalAgentStore):
...
def get_agent_service() -> LocalAgentService:
    """Get the agent service instance."""
    if _agent_service is None:
        raise RuntimeError("Local agent service not initialized")
    return _agent_service


def get_store() -> LocalAgentStore:
    """Get the store instance."""
    if _store is None:
        raise RuntimeError("Local agent store not initialized")
    return _store
...
async def query(request: QueryRequest):
    ...
    """
    logger.info(
        f"Local query endpoint accessed - session_id: {request.session_id or 'new'}, "
    ...
    logger.info("Starting local query stream")
